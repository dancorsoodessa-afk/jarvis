...
from .providers.router import CloudRouterProvider
...
def build_agent(settings: Settings | None = None) -> JarvisAgent:
    ...
    log = get_log("runtime")
    log.info("Старт агента (provider=%s, router=%s)", settings.provider, settings.router_enabled)
    ...
    if settings.provider == "local-vulkan":
        ...
        if settings.router_enabled:
            ...
            provider = OpenAIChatProvider(
                ...
                model=settings.chat_model, history=history,
            )
    else:
    ...
    if session is not None:
        ...
        base_prompt = OpenAIChatProvider().system_prompt

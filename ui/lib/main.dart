...
class _JarvisHomePageState extends State<JarvisHomePage> {
  ...
  Future<void> _connectAndroid() async {
    ...
    if (endpoint.isEmpty || model.isEmpty) {
      if (mounted) setState(() => _status = 'Укажите endpoint и модель');
      return;
    }
  ...
  Future<void> _settings() async {
    ...
    _endpoint.text = _endpoint.text.isEmpty
        ? const String.fromEnvironment('JARVIS_API_URL', defaultValue: '')
        : _endpoint.text;
    _model.text = _model.text.isEmpty
        ? const String.fromEnvironment('JARVIS_MODEL', defaultValue: '')
        : _model.text;
          ...
          child: Column(
            ...
            children: [
              ...
              TextField(
                ...
                obscureText: true,
                decoration: const InputDecoration(
                  labelText: 'API key (необязательно)',
                ),
              ),
              const SizedBox(height: 12),
              const Text(
                'Android работает самостоятельно и не подключается к JARVIS на ПК. '
                'Можно использовать любой OpenAI-compatible AI endpoint.',
        ...
        body: Column(
          children: [
            ...
            const SizedBox(
              width: 140,
              height: 140,
              child: JarvisReactor(color: kCyan),
            ),
            Padding(
                  ...
                  return Align(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: m.isUser ? kCyan.withValues(alpha: .15) : kPanel,
            ...
            if (_busy && _streamText.isNotEmpty)
              Padding(
                padding: const EdgeInsets.all(8),
                child: Text('$_streamText▌'),
              ),
            if (_android && _jarvis == null)
              Padding(
                padding: const EdgeInsets.symmetric(horizontal: 16),
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  IconButton(
                    onPressed: _busy || _jarvis == null ? null : () => _send(_input.text),
                    icon: const Icon(Icons.send, color: kCyan),
                  ),
                  IconButton(
                    onPressed: _busy || _jarvis == null ? null : _clearMemory,

...
class JarvisHomePage extends StatefulWidget {
  ...
  @override State<JarvisHomePage> createState() => _JarvisHomePageState();
}

class _JarvisHomePageState extends State<JarvisHomePage> {
  JarvisIpc? _jarvis;
  final _input = TextEditingController();
  final _host = TextEditingController(text: const String.fromEnvironment('JARVIS_HOST', defaultValue: ''));
  final _port = TextEditingController(text: const String.fromEnvironment('JARVIS_PORT', defaultValue: '8765'));
  ...
  @override
  void initState() { super.initState(); if (!_android) _connectDesktop(); }
  ...
  Future<void> _connectAndroid() async {
    ...
    if (host.isEmpty) { setState(() => _status = 'Укажите IP компьютера'); return; }
    setState(() => _status = 'Подключение к $host:$port…');
    try {
      await _jarvis?.dispose();
      _jarvis = await JarvisIpc.connectTcp(host, port);
      await _finishConnect();
    } catch (e) { setState(() => _status = 'Ошибка подключения: $e'); }
  }

  Future<void> _finishConnect() async {
    final tools = await _jarvis!.listTools();
    if (!mounted) return;
    setState(() => _status = 'JARVIS подключён · инструментов: ${tools.length}');
    _partialSub?.cancel();
    _partialSub = _jarvis!.partials().listen((text) { if (mounted) setState(() => _streamText = text); });
  }

  Future<void> _settings() async {
    if (!_android) return;
    await showDialog<void>(context: context, builder: (ctx) => AlertDialog(
      title: const Text('Подключение к JARVIS'),
      content: Column(mainAxisSize: MainAxisSize.min, children: [
        TextField(controller: _host, keyboardType: TextInputType.url, decoration: const InputDecoration(labelText: 'IP компьютера', hintText: 'например 192.168.1.10')),
        TextField(controller: _port, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Порт')),
      ]),
  ...
  Future<void> _send(String text) async {
    ...
    try {
      ...
      if (reply.needsConfirmation && mounted) {
        final ok = await showDialog<bool>(context: context, builder: (ctx) => AlertDialog(
          title: const Text('Подтверждение'), content: Text(reply.text),
    ...
    } catch (e) { if (mounted) setState(() => _messages.add(_Msg('Ошибка: $e', isUser: false))); }
    finally { if (mounted) setState(() { _busy = false; _streamText = ''; }); }
  }

  Future<void> _clearMemory() async {
    try { await _jarvis!.clearMemory(); if (mounted) setState(() => _messages.add(_Msg('🧹 История диалога очищена.', isUser: false))); }
    catch (e) { if (mounted) setState(() => _messages.add(_Msg('Ошибка: $e', isUser: false))); }
  }

  @override
  void dispose() { _partialSub?.cancel(); _jarvis?.dispose(); _input.dispose(); _host.dispose(); _port.dispose(); _scroll.dispose(); super.dispose(); }
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: Column(children: [
      const SizedBox(height: 8), const SizedBox(width: 140, height: 140, child: JarvisReactor(color: kCyan)),
      Text(_status, style: const TextStyle(color: kCyan, fontSize: 12), textAlign: TextAlign.center),
      ...
      Padding(padding: const EdgeInsets.fromLTRB(12, 4, 12, 12), child: Row(children: [
        ...
        IconButton(onPressed: _busy ? null : () => _send(_input.text), icon: const Icon(Icons.send, color: kCyan)),
